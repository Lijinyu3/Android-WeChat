package com.lijinyu.wechat.profile;
...
import com.lijinyu.wechat.login.LoginActivity;
import com.lijinyu.wechat.password.ChangePasswordActivity;
...
public class ProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (firebaseUser != null) {
            ...
            if (serverFileUri != null) {
                Glide.with(this).load(serverFileUri).placeholder(R.drawable.default_profile).error(R.drawable.default_profile).into(ivProfile);
    ...
    public void btnLogoutClick(View view) {
        ...
        // When users click the back button on the login activity, they will not see this again
    }
    ...
    private void removePhoto() {
        ...
        UserProfileChangeRequest request =
                new UserProfileChangeRequest.Builder().setDisplayName(etName.getText().toString().trim()).setPhotoUri(null).build();
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()) {
                                ...
                                fileRef.delete().addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            ivProfile.setImageDrawable(getDrawable(R.drawable.default_profile));
                                            Toast.makeText(ProfileActivity.this,
                                                    R.string.photo_removed_successfully, Toast.LENGTH_SHORT).show();
                                        } else {
                                            Toast.makeText(ProfileActivity.this,
                                                    R.string.failed_to_update_profile, Toast.LENGTH_SHORT).show();
                                        }
                                    }
                                });
                            } else {
                ...
                if (task.isSuccessful()) {
                    fileRef.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            ...
                            UserProfileChangeRequest request =
                                    new UserProfileChangeRequest.Builder().setDisplayName(etName.getText().toString().trim()).setPhotoUri(serverFileUri).build();
                            ...
                            firebaseUser.updateProfile(request).addOnCompleteListener(new OnCompleteListener<Void>() {
                                ...
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()) {
                                        ...
                                        hashMap.put(NodeNames.PHOTO, photoName);
    ...
    private void updateOnlyName() {
        UserProfileChangeRequest request =
                new UserProfileChangeRequest.Builder().setDisplayName(etName.getText().toString().trim()).build();
        progressBar.setVisibility(View.VISIBLE);
        firebaseUser.updateProfile(request).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
