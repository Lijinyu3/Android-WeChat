...
public class RequestAdapter extends RecyclerView.Adapter<RequestAdapter.RequestViewHolder> {
    ...
    @NonNull
    @Override
    public RequestAdapter.RequestViewHolder onCreateViewHolder(@NonNull ViewGroup parent,
                                                               int viewType) {
    ...
    public void onBindViewHolder(@NonNull RequestAdapter.RequestViewHolder holder, int position) {
        ...
        if (requestModel.getPhotoName().equals("")) {
            Glide.with(context).load(R.drawable.default_profile).into(holder.ivProfile);
        } else {
            StorageReference fileRef = FirebaseStorage.getInstance().getReference().child(Constants.IMAGES_FOLDERS + "/" + requestModel.getPhotoName());
        ...
        holder.btnAcceptRequest.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String requestUserId = requestModel.getUserId();
                String currentUserId = currentUser.getUid();
                databaseReferenceChats.child(currentUserId).child(requestUserId).child(NodeNames.TIME_STAMP).setValue(ServerValue.TIMESTAMP).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            databaseReferenceChats.child(requestUserId).child(currentUserId).child(NodeNames.TIME_STAMP).setValue(ServerValue.TIMESTAMP).addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()) {
                                        databaseReferenceFriendRequests.child(currentUserId).child(requestUserId).child(NodeNames.REQUEST_TYPE).setValue(Constants.REQUEST_STATUS_ACCEPTED).addOnCompleteListener(new OnCompleteListener<Void>() {
                                            @Override
                                            public void onComplete(@NonNull Task<Void> task) {
                                                if (task.isSuccessful()) {
                                                    databaseReferenceFriendRequests.child(requestUserId).child(currentUserId).child(NodeNames.REQUEST_TYPE).setValue(Constants.REQUEST_STATUS_ACCEPTED).addOnCompleteListener(new OnCompleteListener<Void>() {
                                                        @Override
                                                        public void onComplete(@NonNull Task<Void> task) {
                                                            if (task.isSuccessful()) {
                                                                ...
                                                                Toast.makeText(context, R.string.request_accepted_successfully,
        ...
        holder.btnDenyRequest.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String requestUserId = requestModel.getUserId();
                String currentUserId = currentUser.getUid();
                databaseReferenceFriendRequests.child(currentUserId).child(requestUserId).child(NodeNames.REQUEST_TYPE).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            databaseReferenceFriendRequests.child(requestUserId).child(currentUserId).child(NodeNames.REQUEST_TYPE).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
                                @Override
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()) {
